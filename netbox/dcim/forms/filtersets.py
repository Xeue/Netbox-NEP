...
from utilities.forms.fields import ContentTypeChoiceField, ColorField, DynamicModelMultipleChoiceField, TagFilterField
...
class GenericFilterForm(
    ...
    model = Device
    fieldsets = (
        (None, ('q', 'content_types', 'filter_id', 'tag')),
        ('Device', ('device', 'device_type_id', 'status', 'role_id', 'asset_tag', 'manufacturer_id')),
        ('Location', ('site_id', 'location_id', 'rack_id')),
        ('Components', (
            'console_ports', 'console_server_ports', 'power_ports', 'power_outlets', 'interfaces', 'pass_through_ports',
        )),
        ('Miscellaneous', ('has_primary_ip', 'virtual_chassis_member', 'config_template_id', 'local_context_data'))
    )
    content_types = ContentTypeChoiceField(
        queryset=ContentType.objects.filter(FeatureQuery('custom_fields').get_query()),
        required=False,
        label=_('Search Types')
    )
    site_id = DynamicModelMultipleChoiceField(
        queryset=Site.objects.all(),
        required=False,
